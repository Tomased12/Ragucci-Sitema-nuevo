...
export const Tabs: React.FC = () => {
  ...
  const tabs: { id: TabType; label: string; icon: React.ReactNode; isBackup?: boolean }[] = [
    ...
    { id: 'capitales', label: 'Capitales & Caja', icon: <Wallet className="w-3.5 h-3.5 shrink-0" /> },
    { id: 'crm', label: 'Clientes VIP', icon: <Crown className="w-3.5 h-3.5 shrink-0" /> },
    { id: 'configuracion', label: 'Configuración', icon: <Settings className="w-3.5 h-3.5 shrink-0" /> },
    { id: 'backup', label: 'Backup', icon: <HardDriveDownload className="w-3.5 h-3.5 shrink-0" />, isBackup: true },
  ];

  return (
    <nav className="flex flex-wrap items-center justify-between gap-1.5 md:gap-2 mb-6 p-1.5 bg-white rounded-lg shadow-sm border border-ragucci-border">
      {tabs.map((tab) => {
        ...
        return (
          <button
            ...
            className={`flex-1 min-w-[100px] md:min-w-0 flex items-center justify-center gap-1.5 px-2 py-2 md:py-2.5 rounded text-[11px] md:text-xs font-extrabold uppercase tracking-wider transition-all duration-200 cursor-pointer whitespace-nowrap ${
              isActive
